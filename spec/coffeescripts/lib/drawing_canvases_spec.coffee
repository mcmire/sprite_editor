...
describe 'DrawingCanvases', ->
  ...
  describe '#startDrawing', ->
    ...
    it "starts the draw loop", ->
      ...
      expect(canvases.drawTimer).toBe()

    it "doesn't do anything if the draw loop is already running", ->
      canvases.startDrawing()
      timer = canvases.drawTimer
      canvases.startDrawing()
      expect(canvases.drawTimer).toBe(timer)
  ...
  describe '#stopDrawing', ->
    ...
    it "stops the draw loop", ->
      ...
      expect(canvases.drawTimer).not.toBe()

    it "doesn't bomb if the draw loop is not running", ->
      expect(-> canvases.stopDrawing()).not.toThrowAnything()
  ...
  describe '#startSaving', ->
    ...
    it "starts the auto-save loop", ->
      ...
      expect(canvases.autoSaveTimer).toBe()

    it "doesn't do anything if the auto-save loop is already running", ->
      canvases.startSaving()
      timer = canvases.autoSaveTimer
      canvases.startSaving()
      expect(canvases.autoSaveTimer).toBe(timer)
  ...
  describe '#stopSaving', ->
    ...
    it "stops the auto-save loop", ->
      canvases.startSaving()
      canvases.stopSaving()
      expect(canvases.autoSaveTimer).not.toBe()

    it "doesn't bomb if the auto-save loop is not running", ->
      expect(-> canvases.stopSaving()).not.toThrowAnything()
  ...
  describe 'when events have been added', ->
    ...
    describe 'when the mouse is moved', ->
      ...
      it "stores the cells that has focus (considering the brush size)", ->
