...
describe "CellLocation", ->
  ...
  describe '.new', ->
    describe 'given three arguments', ->
      it "accepts an canvases instance and the coordinates within the cells array", ->
        loc = new CellLocation(canvases, 8, 7)
        expect(loc.canvases).toBe(canvases)
        expect(loc.i).toEqual(8)
        expect(loc.j).toEqual(7)
      it "converts i to y and j to x based on the canvases's cell size", ->
        loc = new CellLocation(canvases, 8, 7)
    ...
    describe 'given a single object', ->
      describe 'given a full object', ->
        ...
        it "simply copies the properties over from a plain object", ->
          loc = new CellLocation(canvases: canvases, i: 8, j: 7, x: 10, y: 20)
          expect(loc.i).toEqual(8)
          expect(loc.j).toEqual(7)
          expect(loc.x).toEqual(10)
          expect(loc.y).toEqual(20)
      describe 'given a partial object', ->
        it "uses the canvases's cell size to calculate x from j and y from i", ->
          loc = new CellLocation(canvases: canvases, i: 8, j: 7)
          expect(loc.x).toEqual(70)
          expect(loc.y).toEqual(80)
        it "uses the canvases's cell size to calculate j from x and i from y", ->
          loc = new CellLocation(canvases: canvases, x: 70, y: 80)
          expect(loc.j).toEqual(7)
          expect(loc.i).toEqual(8)
        it "bails if the coordinates are mixed", ->
          expect(-> new CellLocation(canvases: canvases, x: 70, j: 7))
            .toThrow("CellLocation accepts an object containing either i and j or x and y, but not a mixture!")
          expect(-> new CellLocation(canvases: canvases, y: 80, i: 8))
            .toThrow("CellLocation accepts an object containing either i and j or x and y, but not a mixture!")
        it "bails if no coordinates are given", ->
          expect(-> new CellLocation(canvases: canvases))
  ...
  describe '#add', ->
    describe 'given an existing CellLocation object', ->
      it "adds all the coordinates within to the current object", ->
        loc = new CellLocation(canvases: canvases, i: 8, j: 7)
        loc2 = new CellLocation(canvases: canvases, i: 3, j: 1)
        loc.add(loc2)
    ...
    describe 'given a plain object', ->
      ...
      describe 'given x and y coordinates', ->
        it "adds the given coordinates to the current object", ->
          ...
          expect(loc.x).toEqual(80)
          expect(loc.y).toEqual(110)
        it "converts x to j and y to i based on the canvases's cell size", ->
          loc = new CellLocation(canvases: canvases, x: 70, y: 80)
          loc.add(x: 10, y: 30)
          expect(loc.i).toEqual(11)
          expect(loc.j).toEqual(8)
  ...
  describe '#gt', ->
    it "returns true if this.i > other.i", ->
      loc = new CellLocation(canvases: canvases, i: 20, j: 10)
      loc2 = new CellLocation(canvases: canvases, i: 10, j: 10)
      expect(loc.gt(loc2)).toBeTruthy()
    it "returns true if this.j > other.j", ->
      loc = new CellLocation(canvases: canvases, i: 10, j: 20)
      loc2 = new CellLocation(canvases: canvases, i: 10, j: 10)
      expect(loc.gt(loc2)).toBeTruthy()
    it "returns false if this.i == other.i and this.j == other.j", ->
      loc = new CellLocation(canvases: canvases, i: 10, j: 10)
      loc2 = new CellLocation(canvases: canvases, i: 10, j: 10)
      expect(loc.gt(loc2)).toBeFalsy()
    it "returns false if this.i < other.i", ->
      loc = new CellLocation(canvases: canvases, i: 5, j: 10)
      loc2 = new CellLocation(canvases: canvases, i: 10, j: 10)
      expect(loc.gt(loc2)).toBeFalsy()
    it "returns false if this.j < other.j", ->
      loc = new CellLocation(canvases: canvases, i: 10, j: 5)
      loc2 = new CellLocation(canvases: canvases, i: 10, j: 10)
      expect(loc.gt(loc2)).toBeFalsy()
